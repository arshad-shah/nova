...
| id | title | depends_on | risk | est | status |
|---|---|---|---|---|---|
| T-101 | Tauri scaffold: src-tauri workspace, conf, main.rs, dev boot of renderer | T-099 | med | M | open |
...
| T-107 | Edit-role reimplementation (undo/redo/cut/copy/paste/selectAll) | T-104 | med | M | open |
| T-108 | open-external (incl. WSL), fullscreen/reload/devtools, `dialog:*` | T-104 | low | S | open |
| T-109 | File-drop port (`File.path` → Tauri drag-drop event) | T-104 | med | S | open |
...
| T-202 | Config store crate + `settings:*` + `settings:changed` + menu rebuild hook | T-201 | med | M | open |
| T-203 | Keyring crate + `keyring:*` + secret-field keys + profile-secret extract/inject/strip | T-201, T-006 | high | M | open |
| T-204 | `connections:*` (list/save/delete with secret handling) | T-202, T-203 | med | S | open |
| T-205 | AppData store (rusqlite, v1 schema+migrations, WAL, quarantine) + `appdata:*` (15) | T-201 | med | L | open |
| T-206 | Activity log + batcher + `activity:*` + batch/event broadcasts | T-201 | med | M | open |
...
| T-302 | `db:*` handlers (31 channels) over the registry | T-301 | med | M | open |
| T-303 | SQLite driver + parity suite (non-blocking + interrupt) | T-301 | med | L | open |
| T-304 | PostgreSQL driver + parity suite (incl. EXPLAIN plan parser) | T-301 | med | L | open |
| T-305 | MySQL driver + parity suite | T-301 | med | L | open |
| T-306 | MongoDB driver + parity suite (command-syntax translation) | T-301 | high | L | open |
| T-307 | Redis driver + parity suite | T-301 | med | L | open |
...
| T-311 | Export/import glue (`export:*`, `import:*`, dialogs; formats may stub) | T-302 | med | M | open |
...
| id | title | depends_on | risk | est | status |
|---|---|---|---|---|---|
| T-401 | Registry traits: exporter/importer/formatter/type-mapper/command/completion/drag-drop + resolution rules | T-399 | med | M | open |
| T-402 | verql-formats: csv/json/jsonl/sql exporters+importers + formatters (dialect-aware) | T-401 | med | L | open |
| T-403 | verql-themes: model, validation, 9 core themes, `themes:*` + changed event | T-401 | low | M | open |
| T-404 | Plugin manifest model + discovery/validation | T-401 | med | M | open |
| T-405 | Plugin install/uninstall (zip crate; zip-slip/symlink/collision guards) | T-404 | high | M | open |
| T-406 | Permission model + grants persistence + consent parity | T-404 | med | M | open |
| T-407 | `plugins:*` (22) + lifecycle events + declarative plugin loading e2e | T-405, T-406 | med | L | open |
...
| id | title | depends_on | risk | est | status |
|---|---|---|---|---|---|
| T-501 | ToolRegistry + db-tools tools + schema/gating | T-499 | med | M | open |
| T-502 | AI provider clients (anthropic/openai/ollama; SSE; traced network activity) | T-499 | med | L | open |
| T-503 | Conversation manager: prompt assembly, token budget, tool loop, `ai:chat:event` stream parity | T-501, T-502 | high | L | open |
| T-504 | AI permission manager + approvals + app-actions round trip | T-503 | med | M | open |
| T-505 | `ai:*` (24 channels) + conversation persistence wiring | T-503 | med | M | open |
| T-506 | MCP server (rmcp/axum, token auth, SSE-compatible transport, health) + `mcp:*` | T-501 | high | L | open |
| T-507 | MCP write-approval flow + attention integration | T-506, T-504 | med | M | open |
...
| id | title | depends_on | risk | est | status |
|---|---|---|---|---|---|
| T-601 | Updater registry + Homebrew channel + `updater:*` | T-599 | med | M | open |
| T-602 | Packaging: macOS dmg + Homebrew cask script update | T-599 | med | M | open |
| T-603 | Packaging: Linux AppImage | T-599 | med | S | open |
| T-604 | Packaging: Windows (NSIS + MSIX per ADR-0008/T-008 outcome) | T-599, T-008 | high | M | open |
| T-605 | First-run migration UX (detect v1, run importer, report incompatible JS plugins) | T-599 | high | M | open |
| T-606 | Performance comparison vs T-004 baseline + report | T-601..T-605 | med | M | open |
| T-607 | Docs rewrite: docs/, site/ counterparts, CLAUDE.md | T-601..T-605 | med | L | open |
| T-608 | v1 deletion: src/main, src/preload, electron deps, CI cleanup | T-606, T-607 | med | M | open |
