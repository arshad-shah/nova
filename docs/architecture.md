...
| `activity.ts` | the unified activity-stream types (`ActivityKind`, entries) shared by the recorder, the renderer panel, and AI/MCP tool access (see [activity.md](./activity.md)) |
...
| `activity.ts` | mirrors the unified activity stream (cap 1000) that backs the Activity panel; applies each IPC batch in one update (see [activity.md](./activity.md)) |
