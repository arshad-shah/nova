...
export.** The loader (`src/main/plugins/plugin-host.ts`, and identically
`src/main/plugins/isolation/worker-runtime.ts` for isolated plugins) does
`const mod = require(mainPath)` and then checks `typeof mod.activate ===
'function'` directly — there is no `.default` interop anywhere in the loader.
A plain `export default definePlugin({...})` compiles (with esbuild/tsup/tsc
CJS output) to `exports.default = {...}`, not `exports.activate`, so the
plugin fails validation with `'Missing activate() export'`. Destructure the
return value into named exports instead:
...
into a host-defined slot. See [snowflake/index.ts](../src/main/plugins/bundled/snowflake/index.ts)
for a toolbar of role/warehouse selectors driven entirely through declarative
UI, and [ai/index.ts](../src/main/plugins/bundled/ai/index.ts) for the
`ai-chat` panel plus several slot contributions.
