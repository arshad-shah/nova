...
ctx.drivers.register('cassandra', {
  ...
  // `LIMIT n OFFSET m`; 'offset-fetch' ⇒ `OFFSET m ROWS FETCH NEXT n ROWS ONLY`.
  // Omit ⇒ limit-offset. `createRelationalGetTableData` uses it to build the clause.
...
declare the matching capability, or the feature never surfaces.
