...
flowchart TD
  ...
  subgraph main["main process"]
    menu["app-menu.ts native menu<br/>t(node.labelKey)"]
  end
...
The main process never calls `setLocale()`, so it stays on the default `en`
locale and only calls `t()` (in `src/main/app-menu.ts`, the only i18n
call-site in `src/main/`) when building the native menu. There is no wiring
yet to sync the main process to the user's `general.language` setting or to
rebuild the menu on a locale change (English-only today).
...
t('query.results.rows', { count })              // "{count, plural, one {# row} other {# rows}}"
```
...
Plugins can localize their own copy by calling `registerLocale(locale, partial)`
with their namespace — the host never holds plugin strings. No bundled plugin
does this yet: `registerLocale` currently has no call sites outside its own
definition in `shared/i18n/`, and driver-authored copy (connection-field
`label`s, SQL-autocomplete `detail:` text) is still plain hardcoded English in
the driver plugins (e.g. `src/main/plugins/bundled/postgresql/index.ts`), not
routed through any catalogue. If/when that copy is localized, it belongs in a
plugin's own catalogue, not the host's.
